...
const strategyProfiles: Record<SizeLimitedStrategyId, StrategyProfile> = {
  ...
  ultra_fast_h264_nvenc: createProfile({
    overheadRatio: 0.022,
    minOverheadBytes: 24 * 1024,
    maxOverheadBytes: 192 * 1024,
    initialTargetFactor: 0.945,
    preferredAudioBitrate: 72_000,
    minAudioBitrate: 24_000,
    minVideoBitrate: 140_000,
    maxAudioShare: 0.18,
  ...
  ultra_fast_h264_cpu: createProfile({
    overheadRatio: 0.024,
    minOverheadBytes: 24 * 1024,
    maxOverheadBytes: 192 * 1024,
    initialTargetFactor: 0.95,
    preferredAudioBitrate: 72_000,
    minAudioBitrate: 24_000,
    minVideoBitrate: 120_000,
    maxAudioShare: 0.18,
...
export function planSizeLimitedEncode({ targetSizeMb, duration, hasAudio, strategy }: {
  ...
  const planningTargetBytes = Math.max(Math.floor(availableBytes * profile.initialTargetFactor), profile.minOverheadBytes);
  const minTotalBitrate = getMinTotalBitrate({ hasAudio, profile });
  const baseTotalBitrate = Math.max(Math.floor((planningTargetBytes * 8) / safeDuration), minTotalBitrate);
